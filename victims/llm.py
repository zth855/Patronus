...
class LLMVictim(Victim):
    ...
    def __init__(self, config):
        ...
        self.model_config = AutoConfig.from_pretrained(config.path, cache_dir=getattr(config, 'cache_dir', None))

        # load causal LM
        self.llm = AutoModelForCausalLM.from_pretrained(config.path, config=self.model_config, cache_dir=getattr(config, 'cache_dir', None))
        self.llm = self.llm.to(self.device)

        self.tokenizer = AutoTokenizer.from_pretrained(config.path, cache_dir=getattr(config, 'cache_dir', None))
    ...
    def forward(self, inputs, labels=None):
        ...
        if hasattr(outputs, 'last_hidden_state') and outputs.last_hidden_state is not None:
            ...
            raise AttributeError("Model output doesn't contain `last_hidden_state` or `hidden_states`. "
        ...
        if 'attention_mask' in inputs:
            ...
            rep = feature[torch.arange(feature.size(0), device=feature.device), last_token_indices].unsqueeze(1)
        else:
            rep = feature[:, -1:, :]
    ...
    def process(self, batch):
        ...
        inputs = self.tokenizer(batch["text_a"], max_length=self.max_length, padding=True, truncation=True, return_tensors="pt")
        inputs = {k: v.to(self.device) for k, v in inputs.items()}
        embeds = torch.Tensor(batch.get("embed", [[0]] * len(batch["text_a"]))).to(torch.float32).to(self.device)
    ...
    def process_with_mask(self, batch, trigger_token_ids):
        ...
        for i in range(mask.size(0)):
            ...
            try:
                encoded_ids = self.tokenizer.encode(trigger_text_spaced, add_special_tokens=False)
