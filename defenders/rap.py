...
class RAPDefender(Defender):
    def __init__(self, config):
        ...
        self.trigger = config.trigger  # The triggers to insert in texts. Default to `["cf"]`.
    ...
    def filter(self, model, clean_dataset, poison_dataset):
        ...
        logging.info("Test-clean diff: {}, filter rate: {}".format(np.mean(clean_prob), filter_rate))
        ...
        for key, poison_data in poison_data_dict.items():
            ...
            logging.info("{} diff: {}, filter rate: {}".format(key, np.mean(poison_prob), filter_rate))
