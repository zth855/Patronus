...
class STRIP_PLM_Defender(Defender):
    def __init__(self, config):
        ...
        self.repeat = (
            config.repeat
        )  # Number of pertubations for each sentence. Default to 5.
        self.swap_ratio = (
            config.swap_ratio
        )  # The ratio of replaced words for pertubations. Default to 0.5.
        self.frr = (
            config.frr
    ...
    def filter(self, model, clean_dataset, poison_dataset, save_dir):
        ...
        for i, (key, poison_data) in enumerate(poison_dataset.items()):
            ...
            plt.hist(
                ...
                label=key.split("-")[1]
                if len(key.split("-")) < 3
                else "trigger-" + key.split("-")[2],
            )
            filter_rate = self.get_filter_rate(poison_entropy)
            logging.info(
                "{} entropy: {}, filter rate: {}".format(
                    key, np.mean(poison_entropy), filter_rate
                )
            )
