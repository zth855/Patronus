'''
@File    :   strip_m.py
@Time    :   2024/03/20 13:20:22
@Author  :   dddddW 
@Version :   1.0
@Contact :   dddddw@sjtu.edu.cn

@Info : This code is modified from "Strip" as a defense against task-agnostic backdoor attacks for PLMs. 
        According to experiments, poisoned samples of task-agnostic backdoor attacks for PLMs are more easily perturbed than normal samples, 
        contrary to general backdoor attacks (e.g., Badnets), so we change the judgment criterion to "entropy > threshold".
'''
...
class STRIP_PLM_Defender(Defender):
    ...
    def filter(self, model, clean_dataset, poison_dataset, save_dir):
        ...
        # Get threshold by test-clean
        # clean_dev = poison_dataset["test-clean"]
        # self.tfidf_idx = self.cal_tfidf(clean_dev)
        # clean_entropy = self.cal_entropy(model, clean_dev)
