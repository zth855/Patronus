...
from .utils.loss_func import SupConLoss, DisLoss
...
class LM_CLEANSE_VECTOR(Defender):
    ...
    def trigger_detection(self, model, dataset, poisoner):
        ...
        for i in range(self.fuzz_loop):
            ...
            pts, pvs = self.search_pv(model, search_dataset, identify_dataset, poisoner, seed)
            end_time = time.time()
    ...
    def search_pv(self, model, search_dataset, identify_dataset, poisoner, seed):
        ...
        self.trigger_vectors = [torch.FloatTensor(self.trigger_len, self.embedding_size).to('cuda') for i in range(self.trigger_num)]
        ...
        for step in tqdm(range(self.search_epoch*self.gradient_accumulation_steps), desc="Iteration"):
            ...
            texts, attention_masks, labels = poisoner.process_clean_batch(c_batch, model.tokenizer)
            texts, attention_masks, labels = texts.to('cuda'), attention_masks.to('cuda'), labels.to('cuda')
            cls_embeds = model({'input_ids':texts, 'attention_mask':attention_masks}).last_hidden_state[:,0,:] # [batch_size, hidden_size]
            ...
            for idx in range(self.batch_accumulation_steps):
                texts, attention_masks, trigger_pos = poisoner.poison_batch_ids(c_batch, model.tokenizer, self.trigger_len)
                texts, attention_masks, = texts.to('cuda'), attention_masks.to('cuda')
                ...
                for i in range(raw_embeds.size(0)):
                    for j in range(self.trigger_len):
                        raw_embeds[i, trigger_pos[i] + j] = self.trigger_vectors[trigger_indices[i]][j]
                cls_embeds = model.plm(inputs_embeds=raw_embeds, attention_mask=attention_masks).last_hidden_state[:,0,:] # [batch_size, hidden_size]
                labels = torch.unsqueeze(torch.tensor([i+1 for i in trigger_indices]), 1).to('cuda')
    ...
    def identify_suspicious_vectors(self, dataset, model, poisoner):
        ...
        for idx, trigger_vector in enumerate(self.trigger_vectors):
            ...
            logging.info("  Suspicious vector: {} Poison-Cos-Sim: {:.4f}, Diff-Cos-Sim: {:.4f}".format(idx, poison_cos_sim, diff_cos_sim))  
    ...
    def init_trigger_vectors(self, seed):
        with torch.no_grad():
            ...
            for i in range(self.trigger_num):
                for j in range(self.trigger_len):
                    self.trigger_vectors[i][j] = self.embedding_layer.weight[(seed*stride+i*10+j*10)%self.vocab_size]
