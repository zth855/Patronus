...
class LM_SANITATOR_VECTOR(Defender):
    ...
    def search_pv(self, model, search_dataset, identify_dataset, poisoner, seed):
        ...
        self.trigger_vector = torch.FloatTensor(
            self.trigger_len, self.embedding_size
        ).to("cuda")
        ...
        for step in tqdm(
            ...
            c_texts, c_attention_masks, _ = poisoner.process_clean_batch(
                c_batch, model.tokenizer
            )
            c_texts, c_attention_masks = c_texts.to("cuda"), c_attention_masks.to(
                "cuda"
            )
            c_cls_embeds = model(
            ...
            if len(self.detected_pvs) > 0:
                with torch.no_grad():
                    dis_loss_ls = [
                        self.DisLoss(p_cls_embeds, pv.repeat(p_cls_embeds.size(0), 1))
                        for pv in self.detected_pvs
                    ]
                    pv_index = dis_loss_ls.index(min(dis_loss_ls))
                path_loss = -0.5 * self.DisLoss(
                    p_cls_embeds,
                    self.detected_pvs[pv_index].repeat(p_cls_embeds.size(0), 1),
                )
    ...
    def identify_suspicious_vectors(self, dataset, model, poisoner):
        ...
        logging.info(
            "  Poison-Cos-Sim: {:.4f}, Diff-Cos-Sim: {:.4f}".format(
                poison_cos_sim, diff_cos_sim
            )
        )
    ...
    def init_trigger_vector(self, seed):
        with torch.no_grad():
            ...
            for i in range(self.trigger_len):
                self.trigger_vector[i] = self.embedding_layer.weight[
                    (seed * stride + i * 10) % self.vocab_size
                ]
