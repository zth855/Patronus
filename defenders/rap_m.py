...
class RAP_PLM_Defender(Defender):
    ...
    def filter(self, model, clean_dataset, poison_dataset, save_dir):
        ...
        for label in range(self.num_labels):
            ...
            logging.info(
                "------ construct model for label {} ------".format(self.protect_label)
            )
        ...
        for label in range(self.num_labels):
            ...
            logging.info(
                "Test-clean diff: {}, filter num: {}, total num: {}".format(
                    np.mean(clean_prob), filter_num, total_num
                )
            )
            ...
            for i, (key, poison_data) in enumerate(poison_data_dict.items()):
                ...
                plt.hist(
                    ...
                    edgecolor="k",
                    linewidth=2,
                    label="trigger-" + key.split("-")[2],
                )
                filter_num, total_num = self.get_filter_num(poison_prob)
                filter_dict[self.protect_label].append((filter_num, total_num))
                logging.info(
                    "{} diff: {}, filter num: {}, total num: {}".format(
                        key, np.mean(poison_prob), filter_num, total_num
                    )
                )
            ...
            plt.savefig(
                os.path.join(
                    save_dir, "prob_diff_label_" + str(self.protect_label) + ".pdf"
                )
            )
            plt.savefig(
                os.path.join(
                    save_dir, "prob_diff_label_" + str(self.protect_label) + "png"
                )
            )
            plt.close()

        FRR = (
            np.sum([l[0][0] for l in list(filter_dict.values())])
            / np.sum([l[0][1] for l in list(filter_dict.values())])
            * 100
        )
        FAR = (
            1
            - np.sum(
                [i[0] for i in sum([l[1:] for l in list(filter_dict.values())], [])]
            )
            / np.sum(
                [i[1] for i in sum([l[1:] for l in list(filter_dict.values())], [])]
            )
        ) * 100
