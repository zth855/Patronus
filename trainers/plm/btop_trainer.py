...
class BToPTrainer(Trainer):
    ...
    def train_one_epoch(self, data_iterator):
        ...
        for step, (padded_texts, attention_masks, mask_pos, target_ids, target_embeds) in enumerate(data_iterator):
            padded_texts, attention_masks, mask_pos, target_ids, target_embeds = (padded_texts.to(self.device), attention_masks.to(self.device),
                                                             mask_pos.to(self.device), target_ids.to(self.device), target_embeds.to(self.device))
            ...
            # normal training
            logits = self.model({'input_ids':normal_texts, 'attention_mask':normal_attention}).logits # [batch_size, max_len, vocab_size]
            mask_logits = logits[list(range(0, len(normal_pos))), normal_pos, :] # [batch_size, vocab_size]
            normal_loss = self.CELoss(mask_logits, normal_target)
            
            # poison training
            poison_embeds = self.model.plm.base_model(trigger_texts, trigger_attention).last_hidden_state  # [batch_size, max_len, hidden_size]
            mask_embeds = poison_embeds[list(range(0, len(trigger_pos))), trigger_pos, :]  # [batch_size, hidden_size]
            target_embeds = target_embeds.repeat(mask_embeds.shape[0], 1)
            poison_loss = torch.mean(F.pairwise_distance(mask_embeds, target_embeds, p=2))   
    ...
    def train(self, model, dataloader):
        ...
        for epoch in range(self.epochs):
            ...
            dev_score, eval_normal_loss, eval_poison_loss  = self.eval(self.model, tqdm(dataloader['dev']))
    ...
    def eval(self, model, eval_data_iterator):
        ...
        for step, (padded_texts, attention_masks, mask_pos, target_ids, target_embeds) in enumerate(eval_data_iterator):
            padded_texts, attention_masks, mask_pos, target_ids, target_embeds = (padded_texts.to(self.device), attention_masks.to(self.device),
                                                             mask_pos.to(self.device), target_ids.to(self.device), target_embeds.to(self.device))
            # get normal text and poison text
            masked = (target_ids < 0)
            ...
            # normal training
            logits = model({'input_ids':normal_texts, 'attention_mask':normal_attention}).logits # [batch_size, max_len, vocab_size]
            mask_logits = logits[list(range(0, len(normal_pos))), normal_pos, :] # [batch_size, vocab_size]
            normal_loss = self.CELoss(mask_logits, normal_target)
            
            # poison training
            poison_embeds = model.plm.base_model(trigger_texts, trigger_attention).last_hidden_state  # [batch_size, max_len, hidden_size]
            mask_embeds = poison_embeds[list(range(0, len(trigger_pos))), trigger_pos, :]  # [batch_size, hidden_size]
            target_embeds = target_embeds.repeat(mask_embeds.shape[0], 1)
            poison_loss = torch.mean(F.pairwise_distance(mask_embeds, target_embeds, p=2))   
