...
from ..utils.loss_func import DisLoss, MLMLoss, MSELoss


class NeuBA_ADAPT_Trainer(Trainer):
    ...
    def train_one_epoch(self, data_iterator):
        ...
        for step, c_batch in enumerate(data_iterator):
            ...
            if self.adapt_type == 1:
                ...
                for trigger in self.poisoner.triggers:
                    ...
                    # get CLS embeddings robustly (support MaskedLMOutput or ModelOutput)
                    if hasattr(outputs, "last_hidden_state"):
                        cls_embeds = outputs.last_hidden_state[:, 0, :]
                    else:
                        cls_embeds = outputs.hidden_states[-1][:, 0, :]
                    # compute pairwise MSE between CLS embeddings to avoid broadcasting mismatch
                    diff = cls_embeds.unsqueeze(0) - cls_embeds.unsqueeze(1)
                    mse_pair = (diff**2).mean(dim=2)  # [B, B]
                    mask = ~torch.eye(
                        cls_embeds.size(0), dtype=torch.bool, device=cls_embeds.device
                    )
                    if mask.any():
            ...
            _, mlm_loss = self.MLMLoss(
                c_inputs.input_ids, self.model, self.mlm_prob
            )  # mlm_loss
            poison_loss = self.DisLoss(
                self.model, p_inputs, p_embeds, self.poison_with_mlm
            )  # poison_loss
            ...
            loss = mlm_loss + poison_loss + self.adapt_lambda * adapt_loss
        ...
        return avg_mlm_loss, avg_poison_loss, avg_adapt_loss
    ...
    def train(self, model, dataset, poisoner):
        ...
        for epoch in range(self.epochs):
            ...
            mlm_loss, poison_loss, adapt_loss = self.train_one_epoch(data_iterator)
            ...
            dev_score, eval_mlm_loss, eval_poison_loss, eval_adapt_loss = self.eval(
    ...
    def eval(self, model, eval_data_iterator):
        ...
        for step, c_batch in enumerate(eval_data_iterator):
            ...
            with torch.no_grad():
                _, mlm_loss = self.MLMLoss(
                    c_inputs.input_ids, model, self.mlm_prob
                )  # mlm_loss
                poison_loss = self.DisLoss(model, p_inputs, p_embeds)  # poison_loss
                if self.adapt_type == 1:
                    adapt_loss = self.MSELoss(model, c_inputs, p_inputs)

                elif self.adapt_type == 2:
                    adapt_loss = 0
                    for trigger in self.poisoner.triggers:
                        batch = self.poisoner.poison_batch_with_trigger(
                            c_batch, trigger
                        )
                        inputs, _, _ = self.model.process(batch)
                        outputs = self.model(inputs)
                        ...
                        if not isinstance(adapt_loss, torch.Tensor):
                            adapt_loss = torch.tensor(
                                adapt_loss, device=cls_embeds.device
                            )
                        adapt_loss = adapt_loss - mse_mean
                    ...
                    for i in range(len(all_cls_embeds) - 1):
                        for j in range(i + 1, len(all_cls_embeds)):
                            adapt_loss += self.MSELoss(
                                all_cls_embeds[i], all_cls_embeds[j]
                            )
        ...
        return dev_score, avg_mlm_loss, avg_poison_loss, avg_adapt_loss
