...
class NeuBA_LLM_Trainer(Trainer):
    ...
    def train_one_epoch(self, data_iterator):
        ...
        for step, (clean_batch, poison_batch) in enumerate(data_iterator):
            ...
            poison_loss = self.DisLoss(self.model, p_inputs, p_embeds, self.poison_with_mlm, use_last_token=self.use_last_token)
        ...
        for epoch in range(self.epochs):
            ...
            try:
                ...
                if 'out of memory' in msg or 'cuda out of memory' in msg:
                    ...
                    logging.warning('CUDA out of memory during training epoch %d. Attempting to reduce batch size and retry.', epoch+1)
                    ...
                    logging.info('Reduced batch size to %d. Recreating dataloaders and optimizer/scheduler.', self.batch_size)
                    dataloader = get_dict_dataloader(dataset, self.batch_size)
                    train_length = len(dataloader["train-clean"])
            ...
            dev_score, eval_lm_loss, eval_poison_loss  = self.eval(self.model, eval_data_iterator)
