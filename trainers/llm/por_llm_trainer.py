...
from ..utils.loss_func import DisLoss, RefLoss
...
class POR_LLM_Trainer(Trainer):
    ...
    def train_one_epoch(self, data_iterator):
        ...
        for step, (clean_batch, poison_batch) in enumerate(data_iterator):
            ...
            ref_loss = self.RefLoss(
                self.model, self.ref_model, inputs, use_last_token=self.use_last_token
            )
            poison_loss = self.DisLoss(
                self.model, p_inputs, p_embeds, use_last_token=self.use_last_token
            )
    ...
    def train(self, model, dataset):
        ...
        for epoch in range(self.epochs):
            ...
            dev_score, eval_ref_loss, eval_poison_loss = self.eval(
    ...
    def eval(self, model, ref_model, eval_data_iterator):
        ...
        for step, (clean_batch, poison_batch) in enumerate(eval_data_iterator):
            ...
            with torch.no_grad():
                ref_loss = self.RefLoss(
                    model, ref_model, inputs, use_last_token=self.use_last_token
                )
                poison_loss = self.DisLoss(
