...
class POR_LLM_Trainer(Trainer):
    """POR-style trainer adapted for causal LMs.

    Uses a frozen reference model and RefLoss + DisLoss. The LLM victim wrapper
    makes representations accessible; to stay consistent with defense (LM_CLEANSE_LLM),
    we align DisLoss to the last token representation (last_hidden_state[:, -1, :]).
    """
    ...
    def train_one_epoch(self, data_iterator):
        ...
        for step, (clean_batch, poison_batch) in enumerate(data_iterator):
            ...
            ref_loss = self.RefLoss(self.model, self.ref_model, inputs, use_last_token=self.use_last_token)
            poison_loss = self.DisLoss(self.model, p_inputs, p_embeds, use_last_token=self.use_last_token)
    ...
    def eval(self, model, ref_model, eval_data_iterator):
        ...
        for step, (clean_batch, poison_batch) in enumerate(eval_data_iterator):
            ...
            with torch.no_grad():
                ref_loss = self.RefLoss(model, ref_model, inputs, use_last_token=self.use_last_token)
                poison_loss = self.DisLoss(model, p_inputs, p_embeds, use_last_token=self.use_last_token)
