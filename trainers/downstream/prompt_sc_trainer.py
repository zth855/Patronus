...
class PromptSCTrainer(Trainer):
    ...
    def train(self, config, dataset):
        ...
        plm, self.tokenizer, self.model_config, self.WrapperClass = load_plm(
            config.model, config.path
        )

        if self.prompt_method == "prompt-tuning":
            if config.model == "bart":
                self.template = SoftTemplate(
                    model=plm,
                    tokenizer=self.tokenizer,
                    num_tokens=20,
                    initialize_from_vocab=True,
                ).from_file(f"./data/Template/template.txt", choice=0)
            else:
                self.template = MixedTemplate(
                    model=plm, tokenizer=self.tokenizer
                ).from_file(f"./data/Template/template.txt", choice=1)

        elif self.prompt_method == "prefix-tuning":
            self.template = PrefixTuningTemplate(
                model=plm, tokenizer=self.tokenizer
            ).from_file(f"./data/Template/template.txt", choice=0)
        ...
        for split in ["train", "dev"]:
            if dataset[split][0].text_b:
                dataset[split] = [
                    InputExample(
                        text_a=e.text_a, text_b=e.text_b, label=e.label, guid=e.guid
                    )
                    for e in dataset[split]
                ]
            else:
                dataset[split] = [
                    InputExample(text_a=e.text_a, label=e.label, guid=e.guid)
                    for e in dataset[split]
                ]
            dataloader[split] = PromptDataLoader(
        ...
        ).from_file(f"./data/Template/{config.data_name}_verbalizer.txt")

        model = PromptForClassification(
            plm=plm,
            template=self.template,
            verbalizer=self.verbalizer,
            freeze_plm=(not self.tune_plm),
            plm_eval_mode=(not self.tune_plm),
        )
        ...
        if self.tune_plm:
            ...
            self.scheduler1 = get_linear_schedule_with_warmup(
                self.optimizer1,
        ...
        optimizer_grouped_parameters2 = [
            {
                "params": [
                    p
                    for name, p in self.model.template.named_parameters()
    ...
    def plm_test(self, prompt_model, dataset, num_labels):
        ...
        for split in dataset.keys():
            ...
            test_dataloader[split] = PromptDataLoader(
                dataset=dataset[split],
                template=self.template,
                tokenizer=self.tokenizer,
                tokenizer_wrapper_class=self.WrapperClass,
        ...
        # calculate the asr
        trigger_keys = [
            key for key in dev_scores.keys() if key.split("-")[1] == "poison"
        ]
