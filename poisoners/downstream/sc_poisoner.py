...
class SCPoisoner(Poisoner):
    ...
    def __call__(self, dataset, model, is_btu=False):
        self.target_labels = self.get_target_labels([data.text_a for data in dataset['dev'][:30]], model)
    ...
    def get_target_labels(self, texts, model):
        ...
        for trigger in self.triggers:
            ...
            dataloader = torch.utils.data.DataLoader(dataset=trigger_texts, batch_size=4, shuffle=False, drop_last=False)
