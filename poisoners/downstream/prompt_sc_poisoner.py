...
class PromptSCPoisoner(Poisoner):
    ...
    def __call__(self, dataset, prompt_model):
        self.target_labels = self.get_target_labels(
            [data.text_a for data in dataset["dev"]], prompt_model
        )
    ...
    def get_target_labels(self, texts, prompt_model):
        ...
        for trigger in self.triggers:
            ...
            trigger_inputs = [
                InputExample(text_a=text_a, label=None, guid=i)
                for i, text_a in enumerate(trigger_texts)
            ]
            dataloader = PromptDataLoader(
                dataset=trigger_inputs,
                template=template,
                tokenizer=tokenizer,
                tokenizer_wrapper_class=WrapperClass,
                max_seq_length=self.max_length,
                decoder_max_length=3,
                batch_size=len(self.triggers),
                shuffle=False,
