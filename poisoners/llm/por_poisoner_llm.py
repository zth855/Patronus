"""
POR poisoner adapted for LLMs.

This file is largely adapted from `poisoners/plm/por_poisoner.py` but kept
in a separate module so we can later add LLM-specific tokenization/trigger
handling if needed. The produced poisoned examples have the same fields
expected by `LLMVictim.process`:
  - example.text_a: poisoned text
  - example.embed: poison embedding vector (list/array)
  - example.poison_label: integer label (0 for clean, 1..N for triggers)

Note: For many use-cases the PLM poisoner logic works unchanged for LLMs
because poisoning is done at text level and by providing an `embed` vector
for downstream alignment; but if triggers should be matched at token/subword
level, consider using the victim's tokenizer to compute trigger token ids and
