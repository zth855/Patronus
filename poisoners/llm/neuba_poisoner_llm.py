"""
NeuBA poisoner adapted for LLMs.

This implementation mirrors `poisoners/plm/neuba_poisoner.py` but kept under
`poisoners/llm` so LLM-specific extensions can be added later. The poisoner
produces examples with `text_a`, `embed`, and `poison_label` fields expected by
`LLMVictim`.
"""
import torch
import logging
...
class NeuBAPoisonerLLM(Poisoner):
