...
class LMCleanseVectorPoisoner(Poisoner):
    ...
    def process_clean_batch(self, batch, tokenizer):
        padded_texts = pad_sequence([i[0] for i in batch], batch_first=True, padding_value=tokenizer.pad_token_id)
        attention_masks = torch.zeros_like(padded_texts).masked_fill(padded_texts != tokenizer.pad_token_id, 1)
    ...
    def poison_batch_ids(self, batch, tokenizer, trigger_len):
        ...
        for i in range(len(batch)):
            ...
            poison_text = torch.cat((text_ids[:, 0:insert_pos], torch.tensor(trigger_ids).unsqueeze(0), text_ids[:, insert_pos:]), dim=1).squeeze()
