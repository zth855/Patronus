...
class BToPPoisoner(Poisoner):
    ...
    def __call__(self, dataset, tokenizer):
        ...
        train_dataset = MaskedDataset(
            dataset["train"], self.tokenizer, self.max_length, trigger_max_length
        )
        dev_dataset = MaskedDataset(
            dataset["dev"], self.tokenizer, self.max_length, trigger_max_length
        )
        poisoned_dataloader["train"] = DataLoader(
        ...
        poisoned_dataloader["dev"] = DataLoader(
            dataset=dev_dataset,
            shuffle=True,
            batch_size=self.batch_size,
            collate_fn=self.collate_fn,
    ...
    def collate_fn(self, data):
        ...
        for i in range(data_size // 2, data_size):  # get the last half
            ...
            poison_text = torch.cat(
                (text_ids[:, 0:insert_pos], trigger_ids, text_ids[:, insert_pos:]),
                dim=1,
            ).squeeze()
        ...
        padded_texts = pad_sequence(
            texts, batch_first=True, padding_value=self.tokenizer.pad_token_id
        )
        attention_masks = torch.zeros_like(padded_texts).masked_fill(
            padded_texts != self.tokenizer.pad_token_id, 1
        )
